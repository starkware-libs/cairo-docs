...
class TypeSystemVisitor(IdentifierAwareVisitor):
    ...
    def visit_ExprSubscript(self, expr: ExprSubscript) -> Tuple[ExprDeref, CairoType]:
        inner_expr, inner_type = self.visit(expr.expr)
        offset_expr, offset_type = self.visit(expr.offset)

        if isinstance(inner_type, TypeTuple):
            raise CairoTypeError(
                'Subscript-operator for tuples is not supported yet.',
                location=expr.location)

        if not isinstance(inner_type, TypePointer):
            raise CairoTypeError(
                f"Cannot apply subscript-operator to non-pointer type '{inner_type.format()}'.",
                location=expr.location)

        try:
            # If pointed type is struct, get_size could throw IdentifierErrors. We catch and convert
            # them to CairoTypeErrors.
        ...
        if not isinstance(offset_type, TypeFelt):
            raise CairoTypeError(
                ...
                f"'{offset_type.format()}'.", location=offset_expr.location)
        modified_offset_expr = ExprOperator(
            a=offset_expr, op='*', b=element_size_expr, location=expr.location)

        simplified_expr = ExprDeref(
            addr=ExprOperator(a=inner_expr, op='+', b=modified_offset_expr, location=expr.location),
            location=expr.location)
