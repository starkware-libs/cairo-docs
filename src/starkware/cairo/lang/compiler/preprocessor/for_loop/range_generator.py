...
class RangeGeneratorLowering(GeneratorLowering):
    ...
    def init_envelope_iterator(self):
        # TODO(mkaput, 12/05/2022): Assert in runtime that the loop will not go infinite.
        return CodeBlock(), [
            self.start,
        ]

    def condition(self, iterator: ExprIdentifier):
        return CodeBlock([]), BoolEqExpr(eq=True, a=iterator, b=self.stop, location=self.location)
