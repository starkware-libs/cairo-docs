...
def convert_bool_expr_to_jumps(expr: BoolExpr, has_false_case: bool) -> JumpTable:
    ...
    _skip_redundant_jumps_optimization(jump_table)
    _swap_terminals_optimization(jump_table)
    _last_terminal_needs_no_exit_optimization(jump_table)
...
def _skip_redundant_jumps_optimization(jump_table: JumpTable):
    """
    Optimization: If the true case is simply jumping to next instruction, prevent emitting
    unnecessary jump instruction.

    Mutates ``jump_table`` in place.

    Replace::

        ...
        0: JumpToLabelDescriptor(on_true=1, on_false=?)
        1: JumpToLabelDescriptor(X)
        ...

    with::

        ...
        0: JumpToLabelDescriptor(on_true=None, on_false=?)
        1: JumpToLabelDescriptor(X)
        ...
    """

    next_keys = list(jump_table.keys())[1:] + [None]

    for (key, item), next_key in zip(jump_table.items(), next_keys):
        if isinstance(item, JumpToLabelDescriptor):
            if item.on_true == next_key:
                jump_table[key] = dataclasses.replace(item, on_true=None)


def _swap_terminals_optimization(jump_table: JumpTable):
    """
    Optimization: Invert terminals order if we can avoid extra trailing unconditional jump.

    Mutates ``jump_table`` in place.

    Replace::

        0: JumpToLabelDescriptor(on_true=2, on_false=1)
        1: Terminal A
        2: Terminal B

    with::

        0: JumpToLabelDescriptor(on_true=None, on_false=1)
        2: Terminal B
        1: Terminal A

    The simplest case which triggers this optimization is::
    ...
    items = list(jump_table.items())

    (jump_id, jump) = items[-4]
    if not isinstance(jump, JumpToLabelDescriptor):
        return

    (terminal_a_id, terminal_a) = items[-3]
    if not isinstance(terminal_a, Terminal):
        return

    (terminal_b_id, terminal_b) = items[-2]
    if not isinstance(terminal_b, Terminal):
        return

    if jump.on_true == terminal_b_id and jump.on_false == terminal_a_id:
        jump_table[jump_id] = dataclasses.replace(jump, on_true=None)
        jump_table.move_to_end(terminal_a_id)
        jump_table.move_to_end(EXIT_ID)


def _last_terminal_needs_no_exit_optimization(jump_table: JumpTable):
    """
    Optimization: set ``exit`` of the last terminal to None.
    ...
    (last_terminal_id, last_terminal) = list(jump_table.items())[-2]

    if not isinstance(last_terminal, Terminal):
        return

    jump_table[last_terminal_id] = dataclasses.replace(last_terminal, exit=None)
